...
export default function CartPage() {
  ...
  const shipping = resolveShippingPreview(settings, {}, items.length > 0).fee;
  const tax = Math.max(0, subtotal - discountSummary.discount) * 0.02;
  const total = subtotal - discountSummary.discount + shipping + tax;
        ...
        <aside className="glass-panel h-fit rounded-[32px] p-6 shadow-ambient">
          ...
          {settings.promotions?.freeGift?.enabled && (
            <div className="mt-4 rounded-2xl border border-orange-400/20 bg-orange-500/10 px-4 py-3 text-sm text-orange-100">
              Buy {settings.promotions.freeGift.buyQuantity} items to unlock the free gift offer.
            </div>
          )}
          ...
          <Link
            to={items.length ? (isAuthenticated ? "/checkout" : "/auth") : "/"}
            state={items.length && !isAuthenticated ? { from: "/checkout", message: "Please log in to continue to checkout." } : undefined}
            className="mt-6 inline-flex w-full justify-center rounded-2xl bg-brand-500 px-5 py-3 font-semibold text-white"
          >
            {items.length ? (isAuthenticated ? "Continue to checkout" : "Sign in to checkout") : "Browse products"}
          </Link>
        </aside>
      </div>
    </div>
