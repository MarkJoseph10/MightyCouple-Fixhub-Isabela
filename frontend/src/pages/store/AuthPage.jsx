...
export default function AuthPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      navigate(resolvePostAuthRedirect(response?.user, redirectTo), { replace: true });
    } catch (requestError) {
      ...
      <div className="mx-auto max-w-xl glass-panel rounded-[36px] p-8 shadow-ambient">
        ...
        {!isNativeApp ? (
          <div className="mt-4 rounded-2xl border border-cyan-300/15 bg-cyan-400/10 px-4 py-3 text-sm text-cyan-50">
            Desktop access supports account dashboards after sign-in. You can still download the Android app anytime from the download page.
          </div>
        ) : null}
